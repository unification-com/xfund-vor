const { BN, expectEvent, expectRevert, time } = require('@openzeppelin/test-helpers');

const chai = require('chai');
chai.use(require('chai-as-promised'));
const { expect } = chai;

const BlockhashStore = artifacts.require('BlockhashStoreMock');

contract('BlockhashStore', ([owner, alice, bob, carol]) => {
    const blocks = [
        {
            num: 10000467,
            rlpHeader: web3.utils.hexToBytes(
                '',
            ),
            hash: '0x4a65bcdf3466a16740b74849cc10fc57d4acb24cce148665482812699a400464',
        },
        {
            num: 10000468,
            rlpHeader: web3.utils.hexToBytes(
                '',
            ),
            hash: '0x00fd2589a272b85ffaf63223641571bf95891c936b7514ee4e87a593e52de7c9',
        },
        {
            num: 10000469,
            rlpHeader: web3.utils.hexToBytes(
                '',
            ),
            hash: '0x325fde74e261fc483a16506bbc711b645b043ad24c7db0136845a1be262cf0c9',
        },
        {
            num: 10000470,
            rlpHeader: web3.utils.hexToBytes(
                '',
            ),
            hash: '0x5cf096dfd1fc2d2947a96fdec5377ab7beaa0eb00c80728a3b96f0864cec506a',
        },
    ];

    beforeEach(async () => {
        this.blockhashStore = await BlockhashStore.new({ from: owner });

        const [lastBlock] = blocks.slice(-1);
        await this.blockhashStore.godmodeSetHash(lastBlock.num, lastBlock.hash);

        const hash = await this.blockhashStore.getBlockhash(lastBlock.num);
        expect(hash).to.be.equal(lastBlock.hash);
    });

    it('getBlockhash reverts for unknown blockhashes', async () => {
        await expectRevert(this.blockhashStore.getBlockhash(99999999), 'blockhash not found in store');
    });

    it('storeVerifyHeader records valid blockhashes', async () => {
        for (let i = blocks.length - 2; i >= 0; i--) {
            let hash = await this.blockhashStore.getBlockhash(blocks[i + 1].num);
            expect(hash).to.be.equal(web3.utils.sha3(blocks[i + 1].rlpHeader));

            await this.blockhashStore.storeVerifyHeader(blocks[i].num, blocks[i + 1].rlpHeader);
            hash = await this.blockhashStore.getBlockhash(blocks[i].num);
            expect(hash).to.be.equal(blocks[i].hash);
        }
    });

    it('storeVerifyHeader rejects unknown headers', async () => {
        const unknownBlock = blocks[0];
        await expectRevert(
            this.blockhashStore.storeVerifyHeader(unknownBlock.num - 1, unknownBlock.rlpHeader),
            'header has unknown blockhash'
        );
    });

    it('storeVerifyHeader rejects corrupted headers', async () => {
        const [lastBlock] = blocks.slice(-1);
        const modifiedHeader = new Uint8Array(lastBlock.rlpHeader);
        modifiedHeader[137] += 1;
        await expectRevert(
            this.blockhashStore.storeVerifyHeader(lastBlock.num - 1, modifiedHeader),
            'header has unknown blockhash'
        );
    });

    it('store accepts recent block numbers', async () => {
        await time.advanceBlock();

        const n = (await time.latestBlock()) - 1;
        await this.blockhashStore.store(n);

        const hash = await this.blockhashStore.getBlockhash(n);
        const block = await web3.eth.getBlock(n);
        expect(hash).to.be.equal(block.hash);
    });

    it('store rejects future block numbers', async () => {
        await expectRevert(this.blockhashStore.store(99999999), 'blockhash(n) failed');
    });

    it('store rejects old block numbers', async () => {
        for (let i = 0; i < 300; i++) {
            await time.advanceBlock();
        }

        await expectRevert(
            this.blockhashStore.store((await time.latestBlock()) - 256),
            'blockhash(n) failed'
        );
    });

    it('storeEarliest works', async () => {
        for (let i = 0; i < 300; i++) {
            await time.advanceBlock();
        }

        await this.blockhashStore.storeEarliest();

        const n = (await time.latestBlock()) - 256;
        const hash = await this.blockhashStore.getBlockhash(n);
        const block = await web3.eth.getBlock(n);
        expect(hash).to.be.equal(block.hash);
    });
});