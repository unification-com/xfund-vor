...
type IRandomnessRequestStore interface {
	...
	UpdateFulfillment(requestId string, fulfillTxHash string, status string, gasUsed uint64, blockNum uint64, gasPrice uint64) error
	UpdateGasRefund(requestId string, amountRefunded uint64, gasUsed uint64, gasPrice uint64) error
}
