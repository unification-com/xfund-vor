...
// SolidityProof contains precalculations which VOR.sol needs to verifiy proofs
...
// MarshaledProof contains a VOR proof for randomValueFromVORProof.
//
// NB: when passing one of these to randomValueFromVORProof via the geth
...
// MarshalForSolidityVerifier renders p as required by randomValueFromVORProof
...
func UnmarshalSolidityProof(proof []byte) (rv Proof, err error) {
	...
	if len(proof) != ProofLength {
		return failedProof, fmt.Errorf(
			"VOR proof is %d bytes long, should be %d: \"%x\"", len(proof),
			ProofLength, proof)
	}
