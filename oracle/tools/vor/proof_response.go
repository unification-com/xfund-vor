...
// Contains logic/data for mandatorily mixing VOR seeds with the hash of the
// block in which a VOR request appeared
...
// ProofResponse is the data which is sent back to the VORCoordinator, so that
...
type ProofResponse struct {
	...
	// as in FinalSeed.
	P        Proof
	PreSeed  Seed   // Seed received during VOR request
...
// seed is rewritten with the preSeed. (See MarshalForVORCoordinator and
...
// MarshalForVORCoordinator constructs the flat bytes which are sent to the
// VORCoordinator.
func (p *ProofResponse) MarshalForVORCoordinator() (
	...
	if err != nil {
		return MarshaledOnChainResponse{}, errors.Wrap(err,
			"while marshaling proof for VORCoordinator")
	}
	// Overwrite seed input to the VOR proof generator with the seed the
	// VORCoordinator originally requested, so that it can identify the request
	...
	if rl != OnChainResponseLength {
		return MarshaledOnChainResponse{}, errors.Errorf(
			"wrong length for response to VORCoordinator")
	}
	return response, nil
}
...
func (p ProofResponse) CryptoProof(s PreSeedData) (Proof, error) {
	...
	valid, err := proof.VerifyVORProof()
	if err != nil {
...
// GenerateProofResponse returns the marshaled proof of the VOR output given the
...
func GenerateProofResponse(secretKey common.Hash, s PreSeedData) (
	...
	rv, err := p.MarshalForVORCoordinator()
	if err != nil {
		return MarshaledOnChainResponse{}, err
	}
	return rv, nil
