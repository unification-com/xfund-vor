// Package vor provides a cryptographically secure pseudo-random number generator.
...
//
// See VOR.sol for design notes.
//
// Usage
// -----
//
// You should probably not be using this directly.
// oracle/store/core/models/vorkey.PrivateKey provides a simple, more
...
// p.MarshalForSolidityVerifier() to randomValueFromVORProof on the VOR solidity
// contract.
package vor
...
func packUint256s(xs ...*big.Int) ([]byte, error) {
	...
	for _, x := range xs {
		...
		if err != nil {
			return []byte{}, errors.Wrap(err, "vor.packUint256s#EVMWordBigInt")
		}
		mem = append(mem, word...)
	}
	return mem, nil
...
func uint256ToBytes32(x *big.Int) []byte {
	if x.BitLen() > 256 {
		panic("vor.uint256ToBytes32: too big to marshal to uint256")
	}
...
func ScalarFromCurvePoints(
	...
	if !(secp256k1.ValidPublicKey(hash) && secp256k1.ValidPublicKey(pk) &&
		...
		panic("bad arguments to vor.ScalarFromCurvePoints")
	}
...
func (p *Proof) String() string {
	return fmt.Sprintf(
		"vor.Proof{PublicKey: %s, Gamma: %s, C: %x, S: %x, Seed: %x, Output: %x}",
...
func (p *Proof) VerifyVORProof() (bool, error) {
	if !p.WellFormed() {
...
func generateProofWithNonce(secretKey, seed, nonce *big.Int) (Proof, error) {
	...
	if err != nil {
		return Proof{}, errors.Wrap(err, "vor.makeProof#HashToCurve")
	}
