...
COPY ./oracle ./oracle/
COPY ./oracle-cli ./oracle-cli/
RUN cd /root/xfund-vor && make build

COPY ./docker/assets ./oracle/build/
...
CMD cd /root/xfund-vor && \
    npx ganache-cli --deterministic --networkId 696969 --accounts 20 -h 0.0.0.0 --quiet & \
